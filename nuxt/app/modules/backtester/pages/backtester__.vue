...
<template>
  ...
  <UDashboardContent>
    <UDashboardNavbar>
      ...
      <template #right>
        <USelectMenu v-model="selectedColumns" :options="columns" multiple placeholder="Columns" class="w-full" />
        <BGeneralSettings />
      </template>
    </UDashboardNavbar>

    <UTable :sort="sort" v-model="selectedRows" :columns="selectedColumns" :rows="trades" @select="select"
      @contextmenu.prevent="onContextMenu" :loading="IsSaving" :ui="{
        thead: 'sticky',
        tr: {
      ...
      <template #empty-state>
        <div class="flex flex-col items-center justify-center py-6 gap-3">
          <span
            class="font-bold relative inline-block bg-gradient-to-r from-primary via-primary-700 dark:via-primary-200 to-primary bg-clip-text text-transparent drop-shadow-sm">
            No Trading Activity
          </span>
          <div class="text-center font-normal text-gray-500 dark:text-gray-400 text-sm max-w-96"
  ...
  <UContextMenu v-model="isOpen" :ui="{
        ...
        <div class="p-1" v-if="!selectedRows.length || selectedRows.length == 1">
          <UButton color="white" variant="ghost" class="w-full">
            <div class="group flex items-center justify-between gap-1.5 w-full">
              ...
              <UIcon name="heroicons:document-plus"
                class="ml-auto text-gray-400 dark:text-gray-500 ms-auto flex-shrink-0" />
            </div>
          </UButton>
          ...
          <UButton color="white" variant="ghost" class="w-full">
            <div class="group flex items-center justify-between gap-1.5 w-full">
              <div>Edit</div>
              <UIcon name="heroicons:document-text"
                class="ml-auto text-gray-400 dark:text-gray-500 ms-auto flex-shrink-0" />
            </div>
          </UButton>
        ...
        <div class="p-1" v-if="selectedRows.length > 1">
          ...
          <UButton color="white" variant="ghost" class="w-full">
            <div class="group flex items-center justify-between gap-1.5 w-full">
              ...
              <UIcon name="heroicons:document-text-solid"
                class="ml-auto text-gray-400 dark:text-gray-500 ms-auto flex-shrink-0" />
            </div>
          </UButton>
