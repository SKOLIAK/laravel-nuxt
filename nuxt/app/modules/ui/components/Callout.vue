<template>
  ...
  v-bind="attrs" 
  :style="{ '--color-light': colorLight, '--color-dark': colorDark,'--text-light': textLight, '--text-dark': textDark }"
  v-bind:class="gradientBG">
...
const config = computed(() => ({
  wrapper: 'block pl-4 pr-6 py-3 rounded-md border border-[--color-light] dark:border-[--color-dark] bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm/6 my-5 last:mb-0 font-medium group relative prose-code:bg-white dark:prose-code:bg-gray-900',
  ...
  icon: {
    base: 'w-5 h-5 mr-2 inline-flex items-center align-sub text-[--color-light] dark:text-[--color-dark] flex-shrink-0'
  },
  externalIcon: {
...
const textLight = computed(() => {
  if (props.color === 'primary') {
    return 'rgb(var(--color-primary-DEFAULT))'
  }
  // @ts-ignore
  return colors[props.color]?.['600'] || (colors[props.color] as string) || props.color
})
...
const textDark = computed(() => {
  ...
  // @ts-ignore
  return colors[props.color]?.['300'] || (colors[props.color] as string) || props.color
})

const gradientBG = computed(() => {
  return 'bg-gradient-to-bl from-transparent via-transparent to-[--color-dark] dark:to-[--color-light] '
})
