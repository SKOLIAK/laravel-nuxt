        ...
        <component :is="link.children?.length ? DisclosureButton : 'div'" as="template">
          ...
          >
            ...
            <ULink
              ...
              @click="link.click"
            >
            <div class="navlink-active h-4 w-1 rounded rounded-bl-none rounded-tl-none"></div>
              <slot name="icon" :link="link" :is-active="isActive">
                <UIcon
                  v-if="link.icon"
                  :name="link.disabled ? 'fxemoji:lock' : link.icon"
...
const config = computed(() => ({
  wrapper: 'relative !min-h-[auto] !min-w-[auto] px-3',
  container: '!overflow-visible',
  base: 'relative group flex items-center gap-1.5 px-2.5 py-1.5 w-full rounded-md font-medium text-sm focus:outline-none focus-visible:outline-none dark:focus-visible:outline-none focus-visible:before:ring-inset focus-visible:before:ring-2 focus-visible:before:ring-primary-500 dark:focus-visible:before:ring-primary-400 before:absolute before:inset-px before:rounded-md disabled:cursor-not-allowed disabled:opacity-75',
  active: 'text-gray-900 dark:text-white bg-white navlink dark:bg-gray-800 ring-1 ring-inset ring-black/5 dark:ring-white/5',
  inactive: 'filter grayscale text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:before:bg-gray-50 dark:hover:before:bg-gray-800/50',
...
.navlink-active {
  @apply absolute -left-3 top-2 cursor-default
}
