<template>
  <div ref="el" v-bind="{ ...attrs, ...$attrs }" :class="[ui.wrapper, grow ? ui.grow : ui.border, collapsible ? ui.collapsible : 'flex']" :style="{ '--width': width && !grow ? `${width}px` : undefined }">
    ...
    <slot name="handle" :on-drag="onDrag">
      <UDashboardPanelHandle v-if="resizable && !grow" @mousedown="onDrag" :side="side" :isDragged="isDragging"/>
    </slot>
  </div>
...
const config = {
  ...
  border: 'lg:w-[--width] flex-shrink-0',
  grow: 'flex-1',
...
const { el, width, onDrag, isDragging } = props.resizable ? useResizable(id || 'dashboard:panel', { ...(typeof props.resizable === 'object' ? {...props.resizable, ...{ side: props.side}} : {}), value: props.width }) : { el: ref(undefined), width: toRef(props.width), onDrag: undefined, isDragging: undefined }
