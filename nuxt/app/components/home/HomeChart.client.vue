<script setup lang="ts">
  ...
  const { data } = await useAsyncData<DataRecord[]>(
    async () => {
      ...
      let le = dates.map((date, x) => ({
        date,
        amount:
          isLoaded && pnlChartData.value.findIndex((x) => x.date == format(date, "dd.MM.yyyy"))
            ? pnlChartData.value[
                pnlChartData.value.findIndex((x) => x.date == format(date, "dd.MM.yyyy"))
              ].amount
            : 0,
      }));
  ...
  console.warn(
    isLoaded && pnlChartData.value.findIndex((x) => x.date == "05.12.2024")
      ? pnlChartData.value[pnlChartData.value.findIndex((x) => x.date == "05.12.2024")].amount
      : 0
  );
...
<template>
  <UDashboardCard ref="cardRef" :ui="{ body: { padding: '!pb-3 !px-0' } as any }">
    <template #header>
      <div>
        <p class="mb-1 text-sm font-medium text-gray-500 dark:text-gray-400">Cumulated Pnl</p>
