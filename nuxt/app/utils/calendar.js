...
export async function useLoadCalendar() {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const createCalendar = async (param1, param2) => {
      ...
      let dateForCalendarize = new Date(dayjs.unix(param1)).toLocaleString("en-US", {
        timeZone: traderTimeZone.value,
      });
      ...
      let month =
        dayjs(param1 * 1000)
          ...
          .get("month") + 1; //starts at 0 so here we add 1 to get the 'real' month number
      let year = dayjs(param1 * 1000)
        .tz(traderTimeZone.value)
        .get("year");
      ...
      for (let index1 = 0; index1 < calendarizeData.length; index1++) {
        ...
        for (let index = 0; index < element.length; index++) {
          ...
          for (let i = 0; i < filteredTrades.length; i++) {
            ...
            if (
              Number(element.month) == Number(month) &&
              Number(element.year) == Number(year) &&
              Number(element.date) == Number(calData.day)
            ) {
              trade = element;
            }
          }
