...
import { filteredTradesTrades, useChartFormat } from "./global";
import { blotter, executions, existingImports, pAndL, trades, TradesData } from "./trades";
      ...
      await $fetch("/trades", {
        ...
        onResponse({ response }) {
          if (response._data.ok) {
            ...
            console.log(
              " -> Finished getting existing filter trades (" + response._data.data.length + ")"
            );
          }
          resolve();
        },
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const key of keys) {
      try {
        ...
        if (!tradeAccounts.includes(TradesData[key].Account))
          tradeAccounts.push(TradesData[key].Account);
        ...
        temp2.execTime = dayjs
          .tz(formatedDateTD + " " + TradesData[key]["Exec Time"], traderTimeZone.value)
          .unix();
        let tempId =
          "e" +
          temp2.execTime +
          "_" +
          temp2.symbol.replace(".", "_") +
          "_" +
          temp2.type +
          "_" +
          temp2.side;
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const key2 of keys2) {
      ...
      for (let i = 0; i < tempExecs.length; i++) {
        ...
        const existingOpenPositionParseIndex = openPositionsParse.findIndex(
          (x) =>
            x.symbol == tempExec.symbol &&
            x.type == tempExec.type &&
            x.strategy == tempExec.strategy
        );
        /** Checking existing open position amongst open positions stored LOCALLT */
        const existingOpenPositionFileIndex = openPositionsFile.findIndex(
          (x) =>
            x.symbol == tempExec.symbol &&
            x.type == tempExec.type &&
            x.strategy == tempExec.strategy
        ...
        if (newTrade) {
          ...
          if (existingOpenPositionParseIndex != -1) {
            console.log(
              "  --> Open position already in Parse for symbol " +
                key2 +
                " on " +
                useChartFormat(tempExec.td) +
                " at " +
                useTimeFormat(tempExec.execTime)
            );
        ...
        if (newTrade == true) {
          ...
          temp7.id =
            tempExec.side == "B" || tempExec.side == "S"
              ? "t" + tempExec.execTime + "_" + tempExec.symbol + "_" + tempExec.type + "_B"
              : "t" + tempExec.execTime + "_" + tempExec.symbol + "_" + tempExec.type + "_SS";
          ...
          if (tempExec.side == "S") {
            //occasionnaly, Tradezero inverts trades and starts by accounting the sell even though it's a long possition
            temp7.strategy = "long";
            //console.log("Symbol " + key2 + " is sold and long")
            console.log(
              "  --> Symbol " +
                key2 +
                " is accounted as sell before buy on date " +
                useChartFormat(tempExec.td) +
                " at " +
                useTimeFormat(tempExec.execTime)
            );
          ...
          if (tempExec.side == "BC") {
            //occasionnaly, Tradezero invertes trades
            temp7.strategy = "short";
            console.log(
              "  --> Symbol " +
                key2 +
                " is accounted as buy cover before short sell on date " +
                useChartFormat(tempExec.td) +
                " at " +
                useTimeFormat(tempExec.execTime)
            );
          ...
          temp7.executions = [];
          temp7.executions.push(tempExec.id);
          temp7.openPosition = true;
          let exec = executions[tempExec.td].find((x) => x.id == tempExec.id);
          exec.trade = temp7.id;

          console.log(
            "  --> buy quantity " + temp7.buyQuantity + " and sell quantity " + temp7.sellQuantity
          );
          console.log(
            "  --> grossProceeds " +
              temp7.grossProceedsOpen +
              " and netProceeds " +
              temp7.netProceedsOpen
          );
          ...
          if (existingOpenPosition != undefined) {
            ...
            trde = temp2.find(
              (x) => x.id == existingOpenPosition.id && x.td == existingOpenPosition.td
            );
          } else {
          ...
          trde.executions.push(tempExec.id);
          ...
          console.log(
            "  --> buy quantity " + trde.buyQuantity + " and sell quantity " + trde.sellQuantity
          );
          console.log(
            "  --> grossProceeds " +
              trde.grossProceedsOpen +
              " and netProceeds " +
              trde.netProceedsOpen
          );
          ...
          if (openPositionsFile.length > 0)
            openPositionIndex = openPositionsFile.findIndex((x) => x.id == trde.id);
          ...
          if (trde.buyQuantity == trde.sellQuantity) {
            //When buy and sell quantities are equal means position is closed
            //console.log("trde " + JSON.stringify(trde))
            ...
            trde.grossSharePL >= 0
              ? (trde.grossSharePLWins = trde.grossSharePL)
              : (trde.grossSharePLLoss = trde.grossSharePL);
            ...
            trde.netSharePL >= 0
              ? (trde.netSharePLWins = trde.netSharePL)
              : (trde.netSharePLLoss = trde.netSharePL);
    ...
    //console.log("temp2 " + JSON.stringify(temp2))
    var c = _.chain(temp2).orderBy(["entryTime"], ["asc"]).groupBy("td");
...
export async function useCreateBlotter(param) {
  return new Promise(async (resolve, reject) => {
    ...
    let objectZ;
    if (param) {
      // Case when creating blotter for FilteredTrades
    ...
    for (const key9 of keys9) {
      ...
      var z = _.chain(tempExecs).orderBy(["entryTime"], ["asc"]).groupBy("symbol");
      ...
      for (const key10 of keys10) {
        ...
        tempExecs.forEach((element) => {
          ...
          sumFees += Number(
            element.commission + element.sec + element.taf + element.nscc + element.nasdaq
          );
