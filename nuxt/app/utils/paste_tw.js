...
export function pasteTrades(event) {
  return new Promise((resolve, reject) => {
    ...
    if (clip.includes('data-tradingview-clip')) {
      /** Reconstruct */
      let clipObj = JSON.parse(clip.split('data-tradingview-clip="')[1].split('">')[0].replaceAll('&#34;', '"'))
...
function buildPasteObject() {
  return new Promise((resolve, reject) => {
    ...
    if (typeof ClipboardObject.value.sources != undefined &&
      ...
      ClipboardObject.value.sources.forEach(Obj => {
        ...
        if (types.includes(Obj.source.type)) {
          ...
          let contractSpecs = futureContractsJson.value.filter(x => x.symbol == Obj.source.state.symbol.split(':')[1].split('1!')[0])
          ...
          temp.target = getTargetPrice(temp.strategy, temp.entry, Obj.source.state.profitLevel * ticks)
          ...
          temp.outcome = Obj.source.points[3] ? Obj.source.points[3].price == temp.target ? "Win" : Obj.source.points[3].price == temp.stop ? "Loss" : "BE" : 'BE'
          temp.proceeds = temp.outcome == "Win" ? (Obj.source.state.profitLevel * temp.quantity) * value : temp.outcome == "Loss" ? (Obj.source.state.stopLevel * temp.quantity) * -value : 0
