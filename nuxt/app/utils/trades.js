...
import { hasData, filteredTrades, filteredTradesDaily, filteredTradesTrades } from "./global"
import { useCreateBlotter, useCreatePnL } from "./add_trades"
import { useDateTimeFormat, spinnerLoadingPageText, traderTimeZone, totals, totalsByDate } from './global'
...
export async function useGetFilteredTrades(param) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    ***************************************************/
    ...
    console.log(" -> Filtering trades (" + useDateTimeFormat(dateRangeUnix.start, false) + " - " + useDateTimeFormat(dateRangeUnix.end, false) + ")")
      ...
      param1.forEach((value, key) => {
        for (let k in value) {
          ...
          data.trades.forEach(t => {
            ...
            if (
              (dateRangeUnix.value.start == 0 && dateRangeUnix.value.end == 0 ? t.td >= dateRangeUnix.value.start : t.td >= dateRangeUnix.value.start && t.td < dateRangeUnix.value.end)
            ) {
