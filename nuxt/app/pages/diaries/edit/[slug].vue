...
<template>
  ...
  <TiptapEditorContent
    :editor="editor"
    class="block w-full list-disc border-0 bg-white px-0 px-4 py-2 text-sm text-gray-800 focus:ring-0 dark:bg-transparent dark:text-white dark:placeholder-gray-400"
  />

  <TiptapFloatingButton
    icon="octicon:checklist-16"
    @clickUpdateDiary="updateDiary"
    class="ml-4"
    label="Create"
    :loading="userDiaryUpdate === 'pending'"
  >
    <template #content>
      <div class="select-none text-xs font-medium text-gray-500 dark:text-white/20">
        ...
        <span v-if="data.updated_at != data.created_at" class="text-black dark:text-white/40">
          (Modified:
          <AppAtomsUserDateTime :date="data.updated_at" :timezone="authData.user.timezone" />)
        </span>
      </div>
      <div class="ml-2 px-3 py-2">
        <UTooltip text="Mark as important" :shortcuts="['⌘', 'F']">
          <UCheckbox v-model="formData.flagged">
            <template #label>
              <span
                class="text-xsfont-medium select-none"
                :class="{ 'text-gray-500 dark:text-white': data.flagged }"
                v-if="!formData.flagged"
                >Flag</span
              >
              <span
                class="text-xsfont-medium select-none"
    ...
    <template #button> Update Diary </template>
  </TiptapFloatingButton>
</template>
